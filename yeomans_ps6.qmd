...
#davis fix
davis_fix <- atp[grepl("^Davis", tourney_name), tourney_name := "Davis Cup" ]
davis_fix <- atp[ , .(tourney_name = unique(tourney_name))]
davis_fix[, .N]
```
This doesn't quite match the solutions, but it makes sense it is off by three. This is due to the fact that I am still excluding the 2018 date, in which there are three observations. The solutions didn't include this, so it makes sense mine is off by $3$ with a total number of $66$ unique tournaments.
